#!/usr/bin/env python3
"""
Reprocess the M4A track with proper GCS credentials.
"""
...
def reprocess_m4a():
    ...
    try:
        ...
        if result.get("success"):
            print("🎉 M4A track processing completed successfully!")
            print(f"Audio ID: {result.get('audioId')}")
            return result.get('audioId')
        else:
...
if __name__ == "__main__":
    audio_id = reprocess_m4a()
    if audio_id:
        print(f"\n🎵 Track processed successfully! Audio ID: {audio_id}")
        print(f"Embed URL: https://857daa7fb123.ngrok-free.app/embed/{audio_id}")
    else:
