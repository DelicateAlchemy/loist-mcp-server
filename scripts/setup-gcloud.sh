...
# Google Cloud Setup Script for MCP Music Library Server
# This script sets up the necessary Google Cloud services and permissions
...
create_secrets() {
    log "Creating secrets for database passwords..."
    ...
    # Create secrets
    echo -n "$root_password" | gcloud secrets create db-root-password --data-file=- --quiet 2>/dev/null || true
    echo -n "$app_password" | gcloud secrets create db-app-password --data-file=- --quiet 2>/dev/null || true
    
    success "Database password secrets created"
    log "Root password secret: db-root-password"
    log "App password secret: db-app-password"
}
...
# Main execution
main() {
    log "Starting Google Cloud setup for MCP Music Library Server..."
    echo ""
