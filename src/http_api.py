...
def register_http_api_routes(mcp: FastMCP) -> None:
    ...
    # ====================================================================
    # Upload & Ingestion API Endpoints (LOI-45)
    # ====================================================================

    @mcp.custom_route(f"{API_V1_PREFIX}/uploads", methods=["POST"])
    async def create_upload_endpoint(request: Request) -> Response:
        """
        Create an upload and get a GCS signed PUT URL for it.

        POST /api/v1/uploads
        Body: {"filename": "...", "content_type": "audio/mpeg", "size_bytes": 123}
        """
        try:
            body = await request.json()
        except Exception:
        ...
        try:
            result = await upload_service.create_upload(body)
            return JSONResponse({"success": True, **result}, status_code=201)
        except ValidationError as e:
            return error_response(ErrorCode.VALIDATION_ERROR, str(e), 400)
        except Exception as e:
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/uploads/{{uploadId}}/process", methods=["POST"])
    async def process_upload_endpoint(request: Request) -> Response:
        """
        Trigger ingestion of a staged upload. Returns 202 + a job to poll.

        POST /api/v1/uploads/{uploadId}/process
        """
        upload_id = request.path_params.get("uploadId")
        try:
            upload_id = validate_uuid_path(upload_id)
        except ValidationError as e:
            return error_response(ErrorCode.VALIDATION_ERROR, str(e), 400)

        try:
            result = await upload_service.start_processing(upload_id)
            return JSONResponse({"success": True, **result}, status_code=202)
        except ResourceNotFoundError as e:
            return error_response(ErrorCode.RESOURCE_NOT_FOUND, str(e), 404)
        except ValidationError as e:
            return error_response(ErrorCode.VALIDATION_ERROR, str(e), 400)
        except Exception as e:
            logger.exception(f"Failed to start processing for upload {upload_id}: {e}")
            return error_response(ErrorCode.INTERNAL_ERROR, "Internal server error", 500)

    @mcp.custom_route(f"{API_V1_PREFIX}/jobs/{{jobId}}", methods=["GET"])
    async def get_job_endpoint(request: Request) -> Response:
        """
        ...
        try:
            job_id = validate_uuid_path(job_id)
        except ValidationError as e:
            return error_response(ErrorCode.VALIDATION_ERROR, str(e), 400)

        try:
            result = await upload_service.get_job(job_id)
            return JSONResponse({"success": True, **result})
        except ResourceNotFoundError as e:
            return error_response(ErrorCode.RESOURCE_NOT_FOUND, str(e), 404)
        except Exception as e:
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums", methods=["POST"])
    async def create_album_route(request: Request) -> Response:
        """
        Create a new album.

        POST /api/v1/albums
        Body: {"name": "...", "description": "...", "status": "project", ...}
        """
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums", methods=["GET"])
    async def search_albums_route(request: Request) -> Response:
        """
        Search/list albums.

        GET /api/v1/albums?q=nights&limit=20&offset=0&status=released
        """
        from src.services import album_service
        ...
        if not query:
            return error_response(
                ErrorCode.VALIDATION_ERROR, "Query parameter 'q' is required", 400
            )
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}", methods=["GET"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}", methods=["PUT"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}", methods=["DELETE"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}/tracks", methods=["POST"])
    async def add_album_track_route(request: Request) -> Response:
        """
        Add a track to an album.

        POST /api/v1/albums/{albumId}/tracks
        Body: {"audio_track_id": "...", "position": 1, "disc_number": 1}
        """
        from src.services import album_service
        ...
        if not audio_track_id:
            return error_response(
                ErrorCode.VALIDATION_ERROR, "Field 'audio_track_id' is required", 400
            )
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}/tracks/{{audioId}}", methods=["DELETE"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/albums/{{albumId}}/tracks/order", methods=["PUT"])
    async def reorder_album_tracks_route(request: Request) -> Response:
        ...
        if not track_order:
            return error_response(
                ErrorCode.VALIDATION_ERROR, "Field 'track_order' is required", 400
            )
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists", methods=["POST"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists", methods=["GET"])
    async def search_playlists_route(request: Request) -> Response:
        """
        Search/list playlists.

        GET /api/v1/playlists?q=chill&limit=20&offset=0
        """
        from src.services import playlist_service
        ...
        try:
            result = await playlist_service.search_playlists(query, limit=limit, offset=offset)
            return JSONResponse({"success": True, **result})
        except ValueError as e:
            return error_response(ErrorCode.VALIDATION_ERROR, str(e), 400)
        except Exception as e:
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}", methods=["GET"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}", methods=["PUT"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}", methods=["DELETE"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}/tracks", methods=["POST"])
    async def add_playlist_track_route(request: Request) -> Response:
        """
        Add a track to a playlist.

        POST /api/v1/playlists/{playlistId}/tracks
        Body: {"audio_track_id": "...", "position": 1, "added_by": "<userId>"}
        """
        from src.services import playlist_service
    ...
    @mcp.custom_route(
        f"{API_V1_PREFIX}/playlists/{{playlistId}}/tracks/{{audioId}}", methods=["DELETE"]
    )
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}/tracks/order", methods=["PUT"])
    ...
    @mcp.custom_route(f"{API_V1_PREFIX}/playlists/{{playlistId}}/collaborators", methods=["POST"])
    async def add_playlist_collaborator_route(request: Request) -> Response:
        ...
        Body: {"user_id": "...", "role": "viewer"|"editor"}
        """
        from src.services import playlist_service
        ...
        if not user_id:
            return error_response(ErrorCode.VALIDATION_ERROR, "Field 'user_id' is required", 400)
    ...
    @mcp.custom_route(
        f"{API_V1_PREFIX}/playlists/{{playlistId}}/collaborators/{{userId}}", methods=["DELETE"]
    )
