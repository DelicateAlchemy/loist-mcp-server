...
-- Add user_id column to audio_tracks table
...
CREATE INDEX idx_audio_tracks_user_id ON audio_tracks(user_id) WHERE user_id IS NOT NULL;

-- Create composite index for common query pattern (user's tracks by status)
CREATE INDEX idx_audio_tracks_user_status ON audio_tracks(user_id, status) WHERE user_id IS NOT NULL;
