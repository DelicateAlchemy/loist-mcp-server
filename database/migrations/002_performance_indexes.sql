...
CREATE INDEX IF NOT EXISTS idx_audio_tracks_status_created_at
ON audio_tracks(status, created_at DESC);

-- Composite index for format + year (format statistics by year)
CREATE INDEX IF NOT EXISTS idx_audio_tracks_format_year
ON audio_tracks(format, year) WHERE year IS NOT NULL;

-- Composite index for status + format (processing statistics)
CREATE INDEX IF NOT EXISTS idx_audio_tracks_status_format
ON audio_tracks(status, format);

-- Partial index for processing status (frequently queried)
CREATE INDEX IF NOT EXISTS idx_audio_tracks_processing
ON audio_tracks(last_processed_at DESC) WHERE status = 'PROCESSING';

-- Note: Removed recent updates index due to NOW() immutability requirement
-- Consider implementing this as a dynamic query or materialized view if needed
